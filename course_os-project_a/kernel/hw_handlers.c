...
long __attribute__((interrupt("SWI"))) software_interrupt_handler(void)
{
	...
	case SYSCALL_THREAD_CREATE:
		os_printf("Thread create sys call, called");
		//cast it as an int 
		//libc syscall has a return vbalue that is an error code
			//hw-Handlers return the error code 
		//(int)kthread_create((void*)r0, (void*)r1, (void*)r2); 
		//schedule the thread here
		// kthread_handle* tcb = kthread_create((void*)r0, (void*)r1, (void*)r2); 
		/*if (tcb == NULL)
			return -1L; 
		*r0 = tcb; */
		return 0L; 
	case SYSCALL_THREAD_EXIT:
		os_printf("Thread EXIT sys call, called");

		// return kthread_exit(r0); 
	case SYSCALL_THREAD_JOIN:
		os_printf("Thread JOIN sys call, called");

		// return kthread_join(r0, r1); 
	case SYSCALL_THREAD_GET_ID:
		os_printf("Thread GET ID sys call, called");

		// return kthread_get_id(r0); 
	case SYSCALL_THREAD_SELF:		
		os_printf("Thread SELF sys call, called");

		// return kthread_self(r0); 
	default:
