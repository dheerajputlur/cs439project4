 ...
 *///thread is a pointer to a pointer 
int thread_create(thread_t *thread, void *(*func)(void*), void *arg)
{
	//getcurrrentvas()
   return __syscall3(SYSCALL_THREAD_CREATE, (long)thread, (long)func, (long) arg);
}
...
{
	return __syscall2(SYSCALL_THREAD_JOIN, (long)thread, (long)result); 
}
...
{
	return __syscall1(SYSCALL_THREAD_GET_ID, (long)thread); 
}
